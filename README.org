...
- Automatically open the associated media file in MPV based on the filename, open a media file manually with ~C-c C-v~ (~subed-mpv-find-video~), or play media directly from a URL with ~C-c C-u~ (~subed-mpv-play-video-from-url~) .
